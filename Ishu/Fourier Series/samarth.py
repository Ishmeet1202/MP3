...
def getval(expr, **kwargs):
    ...
    for key in kwargs.keys():
        ...
        if ((key == "x") or (key == "xarr") or (key == "xint")):
            ...
            # else:
            #     raise(KeyError("Number of Nodes or Tolerance not found!"))
...
def simpstol(tol, y1, func, h1, a, n):
    n_max = 2**20
    h2 = h1/2
    y2 = np.zeros(2*n)
    result1 = h1*sum(y1)
...
if __name__ == '__main__':
    print("\nValidation: Trapezoidal Method\n")
    repeat = 'y'
    while(repeat == 'y'):
        function = str(input("\nEnter the Function f(x): "))
        # xx = np.linspace(1, 2, 100)
        # t, nn = gaussquad(expression = function, n = 4, m = 1, xarr = xx, tol = 10)
        t, nn = trapz(function, a = 0, b = 1, n = 4, tol = 1)
        # t, nn = simps(function, a = 1, b = 2, n = 2, tol = 10)
        print("Integral is = ", t)
